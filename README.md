...
| 版本 | 日期 | 主题（一句话） | 值不值得升 |
|---|---|---|---|
| **3.1.0** | 2026-09-26 | **让 change 可以活在隔离环境里** —— `--isolate` 三形态；发布规程 + 节点交付物入库；README §6 经验回传口径更正；**修复归档后 `validate`/`status`/`canon verify`/`structure merge` 不可用** | ✅ 需要并行、或 change 会长时间停在只读相位的必升 |
