...
3. 确认 Gate 1（`stdd gate approve <change> --gate 1 --confirmed-by dialog --evidence "用户确认原文"`）— Gate 自动从 YAML 生成 `proposal.md`（Human View）
   - **V3.0.5 硬防线：AI 不得静默自跑 approve；必须先把确认框展示给用户、等用户明确确认后，再带 `--confirmed-by dialog --evidence <用户确认原文>` 执行；不得伪造 evidence。**
   - 若 Gate 1 自动生成未生效，手动执行 `stdd canon generate <change>` 补齐
4. 初始化 `.stdd.yaml` 状态文件（phase: understand → completed）
