...
> Phase 3 (BUILD) 构建循环将连续自动执行。以下是对所有潜在交互点的一次性统一授权。
> 确认后 Phase 3 全程无中断，直到 Gate 3 等待最终确认。
...
以下操作在 Phase 3 期间会被自动执行。请确认是否全部授权：
...
- Phase 3 (BUILD) 完成后，**必须**等待用户确认 `test-report.md` + `design-adjustments.md`
- 长程模式下 Gate 3 不自动跳过
...
授权确认后，系统将自动配置 Claude Code 项目级权限（`.claude/settings.local.json`），添加以下规则以确保 Phase 3 无需逐项手动确认：
...
- **搜索规则**：`Glob`、`Grep`
- **Skill 调用规则**：`stdd-understand`, `stdd-spec`, `stdd-build`, `stdd-deliver`

> 权限配置仅修改项目级 `settings.local.json`，不涉及全局 `~/.claude/settings.json`。Phase 3 完成后可选择恢复原始权限配置。
...
👉 回复「普通模式」切换为常规交互模式（Phase 3 按需暂停交互）。
👉 或逐项回复需要调整的具体项目编号。
```
