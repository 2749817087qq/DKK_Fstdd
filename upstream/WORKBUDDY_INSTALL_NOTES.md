...
- `stdd` — 总入口：判断项目是否已初始化、路由阶段、说明 CLI 位置
- `stdd-understand` — Phase 1 需求理解与确认（Gate 1）
- `stdd-spec` — Phase 2 规格设计与测试方案（Gate 2）
- `stdd-build` — Phase 3 切片规划 + TDD 实现 + 质量验证（Gate 3，V3.0.5 三阶段合一）
- `stdd-deliver` — Phase 4 归档交付
- `stdd-upgrade` — 版本同步与升级
...
**问题**：`/stdd-upgrade`、重新拉取仓库、手动覆盖 skill 文件，都会直接覆盖
`C:\Users\Administrator\.workbuddy-ai\skills\stdd-deliver\SKILL.md`，
...
- 第 2 步：校验策略哨兵 `STDD_LOCAL_POLICY_NO_UPLOAD_V1` 是否仍在位
...
该规程已硬编码写入 `stdd`、`stdd-deliver`、`stdd-upgrade` 三个 skill 的正文中，
...
2. 在 WorkBuddy 对话中直接说需求即可按触发词命中，例如：
   ...
   - 「stdd-spec」/「stdd-build」/「stdd-deliver」可直接进入对应阶段
...
| 测试 | 操作 | 结果 |
|------|------|------|
| 覆盖检测 | 用上游原件替换已适配的 `stdd-deliver/SKILL.md` | `verify` 输出 `[FAIL]` 4 项（哨兵缺失、缺升级规程、残留 `python bin/stdd`、残留相对路径），退出码 1 ✅ |
