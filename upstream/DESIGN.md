...
>
> (V3.0.5：SLICE/BUILD/VERIFY 三阶段合一为 BUILD，六阶段流程 → 四阶段流程 / Six-phase flow → Four-phase flow)
...
2. [四阶段流程 / Four-Phase Flow](#2-四阶段流程--four-phase-flow)
...
Gate 3: Phase 3 (BUILD) 结束 — 用户确认 test-report.md + design-adjustments.md
        End of Phase 3 (BUILD) — User confirms test report + design adjustments
```
...
Gate 2 之后可选择执行模式：
- **全自动长程模式（默认）**：一次性预授权所有 Phase 3 交互点，之后连续自动执行，仅在 Gate 3 等待确认。
- **普通交互模式**：Phase 3 按需暂停交互，保持现有行为。
After Gate 2, execution mode can be selected:
- **Full-Auto Long-Range Mode (default)**: One-time pre-auth for all Phase 3 interaction points, then continuous auto-execution, only pausing at Gate 3.
- **Normal Interactive Mode**: Phase 3 pauses for interaction as needed, maintaining existing behavior.

---

## 2. 四阶段流程 / Four-Phase Flow

> V3.0.5：SLICE / BUILD / VERIFY 三阶段合一的 BUILD LOOP。
> V3.0.5: SLICE / BUILD / VERIFY merged into a single BUILD LOOP.
...
─────────             ────                  ──────────            ───────
  ...
  需求输入          design.md             ┌── 切片规划 ──┐        归档 + 合并
  Input             specs/*.md            │  Slice       │        Archive + Merge
    │               test-plan.md          │  tasks/slices│            │
    ▼                   │                 └─────┬────────┘            ▼
  proposal.md ←── 用户确认(必须)(Gate 1)          ▼                specs 更新
    │              User confirm          ┌── TDD 实现 ──┐         git tag
    ▼                                    │ RED→GREEN   │            │
  用户确认(必须) → 进入 SPEC             │ →REFACTOR   │            ▼
  User confirm → to SPEC                 └─────┬───────┘      完成 / Done
                                            ▼
                                    ┌── 质量验证 ──┐
                                    │ 14类失败模式 │
                                    │ E2E/覆盖率   │
...
### Phase 3: BUILD — 切片规划 + TDD 实现 + 质量验证 | Slice, Build & Quality Verification

> V3.0.5：SLICE / BUILD / VERIFY 三阶段合一为 BUILD。
> V3.0.5: SLICE / BUILD / VERIFY merged into a single BUILD phase.

- **中文**：三个小节顺序推进：
  1. **切片规划 / Slice**：将测试方案拆分为可独立实现的垂直切片。按依赖关系拓扑排序，P0 优先。产出 tasks.md 和 slices.md。自动推进，无需用户确认。
  2. **TDD 实现 / TDD Implementation**：按切片逐一执行 RED → GREEN → REFACTOR。先编写测试（RED），再写最小实现（GREEN），最后重构（REFACTOR）。自动迭代，仅在遇到阻塞或重大设计偏离时暂停询问用户；小偏离记录到 pending-adjustments.md。
  3. **质量验证 / Quality Verification**：全量质量检查 + 设计调整汇总。最多 5 轮迭代（长程 10 轮）：运行 pytest/覆盖率诊断/Lint/类型检查/多版本测试/E2E → Diff 审查 → 十四类失败模式检查 → 汇总设计调整 → 生成 test-report.md。完成后必须等待用户确认（Gate 3）。
- **EN**：Three sub-steps in sequence:
  1. **Slice**: split the test plan into vertical slices. Topological sort by dependencies, P0 first. Produces tasks.md and slices.md. Auto-advances without user confirmation.
  2. **TDD**: RED → GREEN → REFACTOR per slice. Write tests first (RED), minimal implementation (GREEN), then refactor. Auto-iterates; pauses only on blockers or major design deviations; minor deviations recorded to pending-adjustments.md.
  3. **Verify**: full quality check + design adjustment summary. Max 5 iterations (10 long-range): pytest/coverage/lint/typecheck/multi-version/E2E → diff review → fourteen failure mode checks → summarize design adjustments → generate test-report.md. Must wait for user confirmation upon completion (Gate 3).

**十四类失败模式检查 / Fourteen Failure Modes**：
...
| (l) | 锚定缺失 / Anchoring gap | 关键变更缺少参考实现或接口锚定 | V2.7 |
| (m) | Agent CP 失败 / Agent checkpoint failure | Agent 操作序列检查点断言未通过 | V3.0 |
| (n) | 跨系统不一致 / Cross-system inconsistency | 多系统间运行时数据值不一致 | V3.0 |
...
>
> (l) 为 V2.7 新增； (m)-(n) 为 V3.0 新增（Agent 行为验证 + 跨项目知识图谱）。
...
| Gate 3 | Phase 3 (BUILD) 结束 | test-report.md + design-adjustments.md — 测试结果、设计调整 |

**Gate 硬防线 / Gate Hardening（V3.0.5）**：

- 每次确认必须显式声明通道（`stdd gate approve --confirmed-by dialog|file_token|cli`），省略 → exit 2 拒绝。
  Every confirmation must declare its channel (`--confirmed-by dialog|file_token|cli`); omitting it → rejected with exit 2.
- 每次确认落审计链：confirmed_by（通道）+ confirmed_actor（发起者）+ confirmed_evidence（证据）+ confirmed_at（时间戳）。
...
Phase 3 (BUILD) 期间对 Phase 2 设计的任何偏离都必须记录。
Any deviation from the Phase 2 design during Phase 3 (BUILD) must be documented.

**处理流程 / Process**：
```
Phase 3:   发现偏离 → 记录到 pending-adjustments.md
           Deviation found → record to pending-adjustments.md

Phase 3:   小偏离 → 记录继续 / Minor → record & continue
           大偏离 → 暂停，询问用户 / Major → pause, ask user

BUILD 末:  汇总 pending-adjustments → 生成 design-adjustments.md
...
| Phase 3 单切片 TDD / Single slice TDD | 自动完成 | 自动完成 |
| Phase 3 小设计偏离 / Minor design deviation | 记录 pending-adjustments，继续 | 记录 pending-adjustments，继续 |
| Phase 3 大设计偏离 / Major design deviation | 暂停，询问用户 | 自动记录并继续（test-report 汇总） |
| Phase 3 技术阻塞 / Technical blocker | 暂停，询问用户 | 尝试绕过/跳过；无法处理时降级暂停 |
| Phase 3 质量检查失败 / Quality check failure | 自动修复，重新检查 | 自动修复，重新检查 |
| Phase 3 达到迭代上限 / Iteration cap hit | 暂停，向用户报告 | 汇总到 test-report，继续（通过率<95%时降级） |
| Phase 3 完成 / Complete | **强制等待用户确认** | **强制等待用户确认**（不自动跳过） |
| Phase 3 用户有异议 / User disagrees | 回到 Phase 2 | 回到 Phase 2 |

### 3.5 长程模式预授权 / Long-Range Mode Pre-Auth

长程模式下，Gate 2 确认后进入一次性预授权流程，扫描 Phase 3 所有潜在交互点并统一授权：
...
STDD 采用**混合模式**：4 个核心 Skill（流程控制）+ Python CLI 脚本（结构化操作）。
STDD uses a **hybrid model**: 4 core Skills (flow control) + Python CLI scripts (structured operations).
...
│   │   ├── spec.md                 # Phase 2
│   │   ├── build.md                # Phase 3 (V3.0.5: SLICE/BUILD/VERIFY 合一)
│   │   └── deliver.md              # Phase 4
...
│       ├── pending-adjustments.md  # Phase 3 设计偏离记录
│       ├── design-adjustments.md   # Phase 3 设计调整汇总
│       └── test-report.md
│
...
| `/stdd-build` | Phase 3 | 自动 / Auto（V3.0.5: SLICE/BUILD/VERIFY 合一） | 结束时有 / Yes |
| `/stdd-deliver` | Phase 4 | Phase 3 确认后 / After P3 confirm | 无 / No |
...
| design-adjustments.md | Phase 3 | 原始设计引用 + 调整原因 + 影响范围 |
| test-report.md | Phase 3 | 失败项有根因；通过率计算正确 |
...
Phase 3 (BUILD) 开始前，Skill 必须先读取对应开发语言的规范文件。
...
```
                    ...
                    └────┬────┘
                         ...
                         ▼
                    ┌───────────┐
                    │ UNDERSTAND│──→ 用户确认(Gate 1) → completed
                    └────┬──────┘
                         │ /stdd-spec
                         ▼
                    ┌───────────┐
              ┌──→  │   SPEC    │──→ 用户确认(Gate 2) → completed
              │     └────┬──────┘
              │          │ 模式选择 (Gate 2 后)
              │          ▼
              │     ┌─────────────────────┐
              ...
              │     ┌───────────────────────────────┐
              │     │             BUILD             │
              │     │ (SLICE/BUILD/VERIFY 合一)     │──→ 用户确认(Gate 3) → completed
              │     │  切片 → TDD → 质量验证         │     (两种模式均强制)
              │     │  长程:自动处理偏离/10轮         │
              │     └──────────────┬────────────────┘
              │                    │
...
| 版本 | 日期 | 变更内容 |
|------|------|---------|
| **V3.0.5** | 2026-08-17 | 6→4 Phase 合并（SLICE+BUILD+VERIFY → BUILD）+ phase_constants + 旧数据容错 + 版本统一 3.0.5 + Gate 硬防线（--confirmed-by 通道声明 + 审计链 + guard --hook-stdin 路径感知） |
| **V3.0.4** | 2026-07-09 | V3.0 slim plan（6→4 Phase）+ Batch Pipeline 规划 + 目录迁移回归修复 |
| **V2.7** | 2026-06-03 | 结构化基础 + 锚定落地 + 双轨验证 + 上下文工程 + 工程效能优化（V2.6 合并入 V2.7） |
