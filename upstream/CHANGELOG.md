...
## V3.1.0 (2026-09-26) — change 隔离形态 / Change Isolation

> 对应变更 `2026-09-25-new-isolate-flag`。注：**3.0.6 的变更未单独记录**，见 git 历史。
>
> **版本号归并说明（2026-09-26 发布时裁定）**：本段施工期标题为 `V3.0.7`，但 `--isolate`
> 属**新增面向用户能力** ⇒ 按《发布与文档规程》§一语义化表应判 **MINOR**（`3.0.6 → 3.1.0`），
> PATCH（`3.0.6 → 3.0.7`）**不适用**（`--parallel` 移除不构成破坏性变更 —— 它从未在 CLI 注册）。
> 故发布号归并为 **3.1.0**；发布台账以**仓库根 `CHANGELOG.md`** 为准（《规程》§二第 1 件）。
>
> ⚠️ 代码内 `# V3.0.7:` 批注**保留不改**：它们是**施工期批次标签**，且其中一部分属
> **尚未交付**的姊妹 change `2026-09-25-guard-phase-path-scope`，改写会污染历史证据。
> 阅读代码批注时请以本说明为准。
...
### 4-Phase 模型（V3.0.5）
- 阶段模型 6→4：`understand → spec → build → deliver`（原 slice/verify 并入 build）
- Gate 编号保持 1/2/3 不变，Gate 3 语义迁移为「Phase 3 BUILD 质量验收」
- 新增 `phase_constants.py` 单一常量模块（消除 5 处 phase 列表漂移）
- skill 合并：删除 `slice.md` + `verify.md`，内容折叠进新 `build.md`（切片规划 + TDD 实现 + 质量验证三段）
- `spec.md` Step 8 模式选择、`deliver.md` precondition、`_shared/*`、`gates.yaml`、`lite.yaml`、`long_range.yaml`、`AI_OPERATING_MANUAL.yaml` 同步更新

### 旧数据容错
- `LEGACY_PHASE_MAP`（slice/verify → build）+ `_is_build_complete()` 兼容读取旧 6-phase 数据，不重写历史
- slices 证据键统一为 `phases.build.slices_completed`（带 `phase4` 回退）
...
### YAML-first（Slice 2）
- AI 只写 Canonical YAML，Human View MD 在 **Gate 时自动生成**：Gate 1 → `proposal.md`，Gate 2 → `specs/<cap>/spec.md`
- `stdd new` 停止复制 `proposal.md`，改为 scaffold `canonical/proposals/<change>.yaml` + `specs/code/`
- `proposal init` 的 MD→YAML 反向解析**废弃**，降级为 scaffold 别名（带弃用警告）
- Gate hook 跳过 TODO 模板占位（Windows 路径兼容）；spec 模板 scenario 补 `id` 字段

### Batch Pipeline + CLI 分组（Slice 3）
- 批级 mini-STDD 管线：`batch proposal → batch gate --gate 1/2 → batch child add <name> → batch deliver`
- 子 change 继承批级 Gate 1/2 confirmed_at，从 build 起步；deliver 聚合证据 → 批级 test-report.md → 合并 specs
- guard：批级 build 阶段返回活跃子 change（放行代码编辑）；understand/spec 只读提示
- `phase`/`work` 顶层无 change 时回退查 open batch 子 change
- batch `max_items` 从 lite.yaml 读取（5→20）；`add` >5 warn / >10 block（对齐 guard）
- CLI `-h` 按五组渲染：流程 / 管控 / 工具 / 维护 / 批量（子命令回退默认格式）
