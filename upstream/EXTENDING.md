...
STDD Phase 3 (BUILD) 包含 11 类故障模式检查。可通过 Python 代码扩展。
STDD Phase 3 (BUILD) includes 11 failure mode checks, extensible via Python code.
...
3. **更新核心 Skill `build.md`**（V3.0.5：SLICE/BUILD/VERIFY 合并为 BUILD），在质量检查清单中增加新项。
   Update core Skill `build.md` (V3.0.5: SLICE/BUILD/VERIFY merged into BUILD) to add the new check item.
...
| k | 设计偏差未记录 / Undocumented design deviations | build.md Skill |
