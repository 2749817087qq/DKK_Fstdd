    ...
    for change_dir in sorted(changes_dir.iterdir()):
        ...
        if stdd_yaml.exists():
            try:
                ...
                # V3.0.5: use current_phase (fall back to legacy active_phase);
                # compare the final phase NAME, not an integer.
                phase = state.get("current_phase") or state.get("active_phase", "unknown")
            except Exception:
                pass
        changes[change_dir.name] = {
            "status": "completed" if phase == "deliver" else "in_progress",
            "phase": phase,
        }
    return changes
