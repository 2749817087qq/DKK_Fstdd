...
def cmd_proposal_init(args):
    """V3.0.5 (YAML-first): DEPRECATED — MD→YAML 反向解析已废弃。

    YAML 才是源头：AI 直接写 canonical/proposals/<change>.yaml，
    Human View MD 由 `stdd canon generate`（或 Gate 自动）生成。
    init 保留为 scaffold canonical YAML 模板的别名（兼容旧调用）。
    """
    ...
    cmd_canon_init(_argparse.Namespace(
        ...
        project_level=getattr(args, "project_level", False),
    ))
