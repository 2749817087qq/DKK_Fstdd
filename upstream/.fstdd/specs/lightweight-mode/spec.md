...
THEN SHALL 写入 `.fstdd.yaml` 且 `mode: lightweight, score_confidence: preliminary`
...
WHEN 用户选择 `standard`
THEN SHALL 写入 `.fstdd.yaml` 且 `mode: standard`
AND 后续 Phase 按 standard 模式执行

## Requirement: Phase 缩放执行

LITE-REQ-003: Phase 2-6 SHALL 根据 `.fstdd.yaml` 中的 mode 执行不同深度。

### Scenario: lightweight Phase 2 跳过 design 和 test-plan

<!-- confidence: high -->
GIVEN `.fstdd.yaml` 中 `mode: lightweight`
...
<!-- confidence: high -->
GIVEN `.fstdd.yaml` 中 `mode: standard`
WHEN 执行 Phase 2-6
...
<!-- confidence: medium -->
GIVEN `.fstdd.yaml` 中 `mode: lightweight, score_confidence: preliminary`
AND Phase 2 分析发现实际复杂度高于初步评估
WHEN AI 提议上调模式
AND 用户确认
THEN SHALL 更新 `.fstdd.yaml` 中 mode 为 standard
...
<!-- confidence: medium -->
GIVEN `.fstdd.yaml` 中 `task_type: documentation`
WHEN 执行 Phase 5 VERIFY
...
<!-- confidence: medium -->
GIVEN `.fstdd.yaml` 中 `task_type: configuration`
WHEN 执行 Phase 5 VERIFY
