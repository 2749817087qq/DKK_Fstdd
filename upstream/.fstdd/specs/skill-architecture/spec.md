...
`.fstdd/skills/` 下的 6 个 master skill 文件 SHALL 包含 YAML frontmatter（`name` + `description`），使其能够被 Skill 工具独立加载，不再仅是"纸面备份"。

#### Scenario: 所有 master skill 文件含 frontmatter

- **GIVEN** `.fstdd/skills/` 目录存在 6 个文件（understand/spec/slice/build/verify/deliver）
- **WHEN** Skill 工具尝试加载 `.fstdd/skills/understand.md`
...
- **GIVEN** `.fstdd/skills/` 和 `.fstdd/platforms/claude-code/skills/` 中的对应文件
...
系统 SHALL 建立 master→platform 的同步一致性标准：`.fstdd/skills/` 为唯一主版本，platform 目录为派生副本，master 文件修改后 platform 副本 SHALL 同步更新。

#### Scenario: master 修改后 platform 同步

- **GIVEN** `.fstdd/skills/` 为唯一 master 版本
