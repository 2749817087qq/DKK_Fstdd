...
<p align="center">
  <b>V3.0.5</b> &nbsp;·&nbsp; MIT License &nbsp;·&nbsp; Python 3.10+
</p>
...
>
> STDD V3.0 is an **AI-assisted development methodology** powered by Spec-first + TDD execution. 4 ordered phases + 3 mandatory confirmation gates + 14 failure mode checks + cross-project knowledge graph + Agent behavior verification + intelligent Guard + self-learning experience library — transforming vague requirements into high-quality deliverables. Supports 10 languages across 8 AI coding platforms.
...
**STDD (Spec+Test Driven Development)** is an AI-assisted development methodology and tool system. Its core idea: **define behavior first (Spec), write tests second (Test), implement code last** — 4 ordered phases (V3.0.5: SLICE/BUILD/VERIFY merged into BUILD) and 3 mandatory user confirmation gates ensure every line of AI-generated code is traceable and verifiable.
...
Gate 3 ── End of Phase 3 ── User confirms test-report.md + design-adjustments.md (final quality review)
```

After Gate 2, you can choose **Full-Auto Long-Range Mode**: one-time pre-authorization followed by continuous automatic execution of Phase 3 (BUILD), pausing only at Gate 3. Alternatively, choose **Normal Interactive Mode** to pause for interaction at each phase.
...
| **P3: BUILD** ⭐V3.0.5 | Auto | slices.md + tasks.md + TDD impl + test-report.md | **Gate 3** · Mandatory user confirmation |
| **P4: DELIVER** | `/stdd-continue` | archive + merge specs + git tag | None |
...
**Phase 3: BUILD — Slice Planning + TDD Implementation + Quality Verification** (V3.0.5: SLICE/BUILD/VERIFY merged into one)
Slice planning: split the test plan into independently implementable vertical slices (1 spec Scenario → 1+ tests → 1 implementation unit), topologically sorted, P0 first. Then execute RED → GREEN → REFACTOR per slice (tests first, then minimal implementation, then refactor); design deviations are auto-recorded to pending-adjustments.md. Finally run full quality verification: full test suite + coverage diagnostics + Diff review + **12 failure mode checks**, summarize design adjustments into design-adjustments.md, and generate test-report.md.

**Phase 4: DELIVER — Delivery**
...
| **Long-Range Unattended** | One-time pre-auth after Gate 2, Phase 3 (BUILD) executes continuously and automatically, 90%+ operations require no human intervention |
...
| **14 Failure Mode Checks** | Hallucinated actions, scope creep, cascading errors, context loss, tool misuse, runtime deviation, pipeline chain breaks, content quality issues, instruction decay, coverage vacuums, contract gaps, anchor missing, Agent CP failure, cross-system mismatch |
| **Cross-Project Knowledge Graph** ⭐V3.0 | `knowledge-graph.yaml` cross-project pattern extraction + `stdd knowledge merge/query/predict/fix` |
| **Agent Behavior Verification** ⭐V3.0 | `stdd agent verify` CP checkpoint validation + Phase 3 (Build) dual pipeline (code + Agent) |
| **Bootcamp** ⭐V3.0.3 | AI training camp: 5-level drills + auto-grading + muscle memory |
| **Intelligent Guard** ⭐V3.0 | Tiered reminders: allow small + suggest medium + block large with STDD guidance |
| **Coverage Gap Analysis** | `stdd diff` outputs a spec-to-test coverage gap comparison table |
| **Design Adjustment Traceability** | Design deviations auto-recorded to design-adjustments.md, reviewed at Gate 3 |
| **25-Command CLI** | init / install / new / validate / status / archive / trace / diff / rollback / abort / experience / ci / gate / state / curate / proposal / canon / index / agent / hooks / structure / skill / fix / guard / knowledge |
...
| (m) | Agent CP failure | Agent checkpoint assertion not passed during operation sequence | V3.0 |
| (n) | Cross-system mismatch | Runtime data value inconsistency between multiple systems | V3.0 |
...
Just say this in your AI coding platform:

```
"Install https://github.com/leonai42/stdd as a global skill"
```

```
"/STDD init this project"
```

```
"/stdd develop We need to add rate limiting to the API"
```

The AI handles installation, initialization, upgrades, and the full 4-phase flow automatically. You only need to confirm at the three Gates.

### Method 2: CLI Commands (for scripting/CI)
...
| `stdd knowledge <sub>` | Cross-project knowledge graph (merge/query/predict/fix) | V3.0 |
...
Language-specific development standards are auto-loaded before Phase 3 (BUILD) begins. V2.9.6 covers 10 languages:
...
A mid-to-short-term trend strategy system built with STDD methodology, iterated to stable V4.2. 19,500+ lines of test code, multi-layer scoring decision system with multi-level risk controls. Complete changes/ + specs/ directory structure; every strategy iteration goes through the full four-phase process.

### Visio Flowchart Skill — Small Project Example

Completed in under 1 hour with the full STDD four-phase process: 271 lines of core deliverables, 29 TC at 100% pass rate. Proves STDD isn't just for medium-to-large projects — small tasks benefit equally, with no compromise on quality regardless of scale.
...
|---------|------|-------------|
| **V3.0.5** | 2026-08-17 | 6→4 Phase merge (SLICE+BUILD+VERIFY → BUILD) + phase_constants + legacy tolerance + version 3.0.5 + Gate hardening (--confirmed-by + audit chain + guard path-awareness) |
| **V3.0.4** | 2026-07-09 | V3.0 slim plan (6→4 Phase) + Batch Pipeline planning + directory migration regression fixes |
| **V3.0.0** | 2026-07-02 | Cross-project knowledge graph + Agent behavior verification + intelligent Guard + 14 failure modes |
