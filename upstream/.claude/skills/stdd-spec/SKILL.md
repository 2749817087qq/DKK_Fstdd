...
✏️ 其他建议（自由输入）
   · 对执行模式、预授权范围、流程本身有其他想法
   · 直接说明，AI 必须采纳并复述确认后再继续
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

**交互铁律（D哥 2026-09-18 反馈）**：任何 AskUserQuestion 选项清单**必须**
包含「其他（自由输入）」项。选项卡是快捷入口，不是封闭单选——用户若有
预设之外的意图，必须有第一-class 的自由文本通道。若所用客户端的选项卡
UI 不支持自由输入，则**不得以选项卡提问**，改用纯文本提问
（聊天回复天然是自由文本）。

→ 用户选择长程模式：进入 Step 8a 一次性预授权流程
→ 用户选择普通模式：进入 Step 8b，直接启动 Phase 3
→ 用户选择其他/自由输入：复述用户意图并确认理解无误，按其建议调整
  （涉及授权范围的调整写入 pre_auth_timestamp 备案）后继续
