...
- **thorough**：全部 14 类 + subagent 交叉验证 + Agent 验证管线并行
...
> 以下 (l) 为 V2.7 新增，(m)(n) 为 V3.0 新增。

**(l) 锚定缺失** — critical Change 未达到锚定等级要求

- 来源：V3.0 提案 #8
- 定义：proposal 标记为 `critical: true` 的 Change，其 `anchoring.level` 未达到最低要求
- 检查时机：Phase 2 Gate 2，spec 确认前
- 处理：Gate 2 阻塞，补充锚定后放行

**(m) Agent 检查点失败** — Agent 操作序列中 CP 断言未通过

- 来源：V3.0 Change B
- 检查：`agent_spec.yaml` 中定义的检查点是否全部通过？
- 典型信号：
  - CP 断言失败（操作完成后环境状态与预期不一致）
  - CP 执行超时（Agent 在某步卡住）
  - CP 依赖链断裂（上游 CP 未通过，下游全部跳过）
- 检查方法：执行 `stdd agent verify <change>`，逐 CP 验证状态断言
- 处理：阻断 Change，追溯到失败 CP 判断是 spec 错误还是 Agent 执行错误

**(n) 跨系统不一致** — 多系统间运行时数据值不一致

- 来源：V3.0 Change B
- 检查：`agent_spec.yaml` 中 `cross_check` 断言是否通过？
- 典型信号：
  - CRM 和财务系统客户名称不一致
  - 前端状态和后端数据库不同步
  - Agent 多步操作后早期状态被后续操作覆盖
- 检查方法：执行 `stdd agent verify <change>`，对比 cross_check 中 source 和 target 的实际值
- 处理：阻断 Change，报告不一致的具体字段和两个系统的实际值
...
### Step 3.3: Agent 验证管线并行执行（V3.0 — thorough 模式）

在 Step 3 执行失败模式检查的同时，并行启动 Agent 验证管线：

1. 如果 `specs/agent/` 或 change 目录中存在 `agent_spec.yaml`：
   - 执行 `python bin/stdd agent verify <change>` 
   - CP 逐条验证 + cross_check 跨系统比对
2. 生成 `agent-verify-report.md`（CP 状态：PASS/FAIL/SKIP）
3. Agent 验证失败触发 (m) 或 (n) 失败模式
4. 如果无 `agent_spec.yaml` → 跳过此步骤

**Agent 验证与代码验证的关系**：
- 代码验证（pytest + lint + 失败模式 a-l）→ 验证产物正确性
- Agent 验证（CP check + cross_check）→ 验证操作后世界状态正确性
- 两者并行执行，结果汇总到 test-report.md
- 任一失败均阻断 Change

### Step 3.5: 自动记录/更新经验

在完成失败模式检查后，将本次发现的失败模式记录到项目经验库，供后续 BUILD 阶段复用：
