...
>
> STDD V3.0 是一套 **Spec 先行 + TDD 执行** 的 AI 辅助研发流程系统。4 个有序阶段 + 3 道强制确认门 + 14 类失败模式检查 + 跨项目知识图谱 + Agent 行为验证 + 智能 Guard + 自学习经验库，将模糊需求转化为高质量交付。支持 10 门语言，适配 8 大 AI 编程平台。
...
**STDD (Spec+Test Driven Development)** 是一套 AI 辅助的研发流程方法论与工具系统。它的核心思想是：**先定义行为（Spec），再编写测试（Test），最后实现代码** — 通过 4 个有序阶段（V3.0.5：SLICE/BUILD/VERIFY 合一）和 3 道强制用户确认门，确保 AI 生成的每一行代码都有据可查、有测可验。
...
Gate 3 ── Phase 3 结束 ── 用户确认 test-report.md + design-adjustments.md（质量终审）
```

Gate 2 之后可选择**全自动长程模式**：一次性预授权后，Phase 3（BUILD）连续自动执行，仅 Gate 3 等待确认。也可选择**普通交互模式**，每阶段按需暂停。
...
| **P3: BUILD** ⭐V3.0.5 | 自动执行 | slices.md + tasks.md + TDD 实现 + test-report.md | **Gate 3** · 强制用户确认 |
| **P4: DELIVER** | `/stdd-continue` | archive + merge specs + git tag | 无 |
...
**Phase 3: BUILD — 切片规划 + TDD 实现 + 质量验证**（V3.0.5：SLICE/BUILD/VERIFY 三阶段合一）
切片规划：将测试方案拆分为可独立实现的垂直切片（1 个 spec Scenario → 1+ 测试 → 1 个实现单元），按依赖拓扑排序、P0 优先。然后逐切片执行 RED → GREEN → REFACTOR 循环（先写测试、再最小实现、最后重构），设计偏离自动记录到 pending-adjustments.md。最后全量质量验证：全量测试 + 覆盖率诊断 + Diff 审查 + **12 类失败模式检查**，汇总设计调整到 design-adjustments.md，生成 test-report.md。
...
| **长程无人值守** | Gate 2 后一次性预授权，Phase 3 (BUILD) 全自动连续执行，90%+ 操作无需人工干预 |
...
| **14 类故障检查** | 幻觉行为、范围蔓延、级联错误、上下文丢失、工具误用、运行时偏差、管线断链、内容质量偏差、指令衰减、覆盖真空、契约断层、锚定缺失、Agent CP 失败、跨系统不一致 |
| **跨项目知识图谱** ⭐V3.0 | `knowledge-graph.yaml` 跨项目失败模式提炼 + `stdd knowledge merge/query/predict/fix` 四命令 |
| **Agent 行为验证** ⭐V3.0 | `stdd agent verify` CP 检查点逐条验证 + Phase 3 (Build) 代码验证与 Agent 验证双管线并行 |
| **Bootcamp 训练营** ⭐V3.0.4 | AI 新兵训练营：5关实战演练 + 自动评分 + 肌肉记忆 |
| **智能 Guard** ⭐V3.0 | 分级提醒模式：小改放过 / 中改建议 / 大改拦截引导 STDD 流程 |
| **覆盖差异分析** | `stdd diff` 输出 spec 与测试之间的覆盖缺口对比表 |
| **设计调整追溯** | 设计偏离自动记录到 design-adjustments.md，Gate 3 统一审核 |
| **25 命令 CLI 工具** | init / install / new / validate / status / archive / trace / diff / rollback / abort / experience / ci / gate / state / curate / proposal / canon / index / agent / hooks / structure / skill / fix / guard / knowledge |
...
| (m) | Agent CP 失败 | Agent 操作序列检查点断言未通过 | V3.0 |
| (n) | 跨系统不一致 | 多系统间运行时数据值不一致 | V3.0 |
...
AI 会自动处理安装、初始化、升级、以及完整的 6 Phase 流程。你只需要在三个 Gate 确认即可。

### 方式二：CLI 命令（脚本/CI 场景）

<details>
<summary>点击展开 CLI 命令</summary>
...
| `stdd knowledge <sub>` | 跨项目知识图谱（merge/query/predict/fix） | V3.0 |
...
基于 STDD 方法论的中短线趋势策略系统，已迭代至 V4.2 稳定版。19,500+ 行测试代码，多层评分决策体系 + 多级风控防护。完整的 changes/ + specs/ 目录，每次策略迭代走完整四阶段流程。

### Visio Flowchart Skill — 小型项目

1 小时以内，完整 STDD 四阶段流程，271 行核心交付件，29 TC 100% 通过率。证明 STDD 不只适用于中大型项目 — 小型任务同样能从中获益，质量不因规模而妥协。
