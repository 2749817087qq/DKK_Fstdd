...
def _migrate_dirs_to_stdd(project_root: Path) -> None:
    ...
    for old_name, new_name in _OLD_TO_NEW.items():
        ...
        if old_path.exists() and not new_path.exists():
            _shutil.move(str(old_path), str(new_path))
            ...
            try:
                old_path.rmdir()  # remove if empty
                print(f"  [迁移+合并] {old_name}/ → {new_name}/")
            except OSError:
...
def _cmd_upgrade_current(args: argparse.Namespace) -> None:
    ...
    _write_upgrade_notes(project_root, proj_ver, source_ver)
