...
# V3.0.5: CLI 命令按场景分组（流程/管控/工具/维护/批量）
COMMAND_GROUPS = [
    ("流程", ["new", "validate", "status", "phase", "gate", "work", "state",
...
_CMD_HELP = {
    "init": "初始化 STDD 到当前项目",
    "new": "创建新的 change 目录骨架",
    "validate": "验证 change 结构",
    "status": "显示 artifact 完成状态",
    "archive": "归档已完成的 change",
    "rollback": "从 archive 恢复已归档的 change",
    "abort": "放弃变更并归档到 aborted/",
    "trace": "查看 spec<->test<->code 追溯链",
    "diff": "显示 spec<->test<->code 覆盖差异",
    "extract-proposal": "从 proposal 提取结构化数据",
    "dependency-graph": "构建 spec 依赖图",
    "ci": "CI/CD 集成管理",
    "experience": "管理项目级 AI 经验库",
    "state": "查看/恢复跨 Session 状态",
    "gate": "Gate 确认管理 (1/2/3)",
    "proposal": "管理 Canonical proposal",
    "canon": "双轨制文档管理 (YAML-first)",
    "index": "项目索引管理",
    "agent": "Agent 行为验证",
    "hooks": "生命周期 Hooks 管理",
    "structure": "代码结构摘要管理",
...
class _STDDParser(argparse.ArgumentParser):
    """V3.0.5: format_help 按场景分组渲染命令列表（流程/管控/工具/维护/批量）。"""

    def format_help(self) -> str:
        # 仅顶层解析器分组渲染；子命令（prog 含空格，如 "__main__.py batch"）回退默认格式
        if " " in self.prog:
        ...
        for a in self._actions:
            if a.dest == "help":
                ...
                lines.append("  --dry-run          预览操作，不实际修改文件系统")
            elif a.dest == "verbose":
...
def main() -> None:
    ...
    # V3.0.5 硬防线：--confirmed-by 必填，无默认值
    ...
    # V2.9/V3.0.5: batch — lightweight change batch + batch pipeline (mini-STDD)
    p_batch = subparsers.add_parser("batch", help="轻量变更批次管理 + 批级管线 (V2.9/V3.0.5)", parents=[parent])
    batch_subs = p_batch.add_subparsers(dest="action")
    p_b_open = batch_subs.add_parser("open", help="打开批次", parents=[parent])
    p_b_open.add_argument("description", nargs="?", default="", help="批次描述")
    p_b_open.add_argument("--strategy", choices=["monthly", "weekly", "count_based"],
                          default="monthly", help="批次策略 (默认: monthly)")
    p_b_add = batch_subs.add_parser("add", help="添加项到当前批次", parents=[parent])
    p_b_add.add_argument("description", help="修复描述")
    p_b_close = batch_subs.add_parser("close", help="闭合批次", parents=[parent])
    p_b_close.add_argument("--force", action="store_true", help="强制闭合 (跳过近空批次保护)")
    batch_subs.add_parser("archive", help="归档批次", parents=[parent])
    batch_subs.add_parser("list", help="列出批次", parents=[parent])
    batch_subs.add_parser("status", help="批次状态", parents=[parent])
    # V3.0.5: batch pipeline (mini-STDD)
    p_b_proposal = batch_subs.add_parser("proposal", help="写批级 proposal + design (管线)", parents=[parent])
    p_b_proposal.add_argument("description", nargs="?", default="", help="批级描述/标题")
    p_b_gate = batch_subs.add_parser("gate", help="批级 Gate 确认 (管线)", parents=[parent])
    p_b_gate.add_argument("--gate", type=int, choices=[1, 2], required=True, help="Gate 编号 (1=理解 2=规格)")
    # V3.0.5: 批级 Gate 同样强制 confirmed_by 通道声明
    p_b_gate.add_argument("--confirmed-by", required=True, choices=["dialog", "file_token", "cli"],
                          help="确认通道 (必填): dialog=用户口头确认 / file_token=批级 token / cli=直接命令")
    p_b_gate.add_argument("--evidence", default="", help="确认证据（如用户确认原文）")
    p_b_child = batch_subs.add_parser("child", help="批级子 change 管理 (管线)", parents=[parent])
    child_subs = p_b_child.add_subparsers(dest="child_action")
    p_b_child_add = child_subs.add_parser("add", help="创建子 change", parents=[parent])
    p_b_child_add.add_argument("name", help="子 change 名称")
    p_b_child_add.add_argument("description", nargs="?", default="", help="子 change 描述")
    child_subs.add_parser("list", help="列出子 change", parents=[parent])
    p_b_deliver = batch_subs.add_parser("deliver", help="批级交付聚合 (管线)", parents=[parent])
    # V3.0.5: 批级 deliver 的 Gate 3 强制 confirmed_by 通道声明（不再静默自动确认）
    p_b_deliver.add_argument("--confirmed-by", required=True, choices=["dialog", "file_token", "cli"],
                             help="Gate 3 确认通道 (必填): dialog=用户口头确认 / file_token=批级 token / cli=直接命令")
    p_b_deliver.add_argument("--evidence", default="", help="确认证据（如用户确认原文）")
    ...
    p_guard.add_argument("--quiet", "-q", action="store_true",
                         help="静默模式")
    # V3.0.5: --hook-stdin — hook 模式从 stdin 读 PreToolUse JSON 拿目标路径（路径感知）
    p_guard.add_argument("--hook-stdin", action="store_true",
                         help="从 stdin 读 PreToolUse hook JSON 解析目标文件路径 (路径感知)")
    ...
    # V3.0: knowledge — cross-project knowledge graph
    p_knowledge = subparsers.add_parser("knowledge", help="跨项目知识图谱 (V3.0)", parents=[parent])
    p_knowledge.add_argument("subcommand", nargs="?", choices=["merge", "query", "predict", "fix"],
                             default="query", help="子命令 (默认: query)")
    p_knowledge.add_argument("keyword_or_id", nargs="?", default="",
                             help="query: 关键词 / predict: change名称 / fix: EXP-ID")
    p_knowledge.add_argument("--change-name", dest="change_name", default="",
                             help="predict: change 名称")
    p_knowledge.add_argument("--experience-id", dest="experience_id", default="",
                             help="fix: 经验 ID")
    p_knowledge.add_argument("--related", action="store_true",
                             help="query: 同时显示关联节点")
    p_knowledge.add_argument("--format", choices=["table", "json"], default="table",
                             help="输出格式 (默认: table)")

    # V3.0.x: bootcamp — AI training camp
    p_bootcamp = subparsers.add_parser("bootcamp", help="AI训练营 (V3.0.x)", parents=[parent])
    p_bootcamp.add_argument("subcommand", nargs="?", choices=["start", "status", "retry", "skip", "grade"],
                            default="status", help="子命令 (默认: status)")
    p_bootcamp.add_argument("level", nargs="?", type=int, default=None,
                            help="retry/skip/grade: 关卡编号 (1-5)")
    p_bootcamp.add_argument("--auto", action="store_true", default=False, help="自动完成所有关卡训练")
    ...
    commands = {
        "init": "fstdd.cli.commands.init.cmd_init",
        "new": "fstdd.cli.commands.new.cmd_new",
        "validate": "fstdd.cli.commands.validate.cmd_validate",
        "status": "fstdd.cli.commands.status.cmd_status",
        "archive": "fstdd.cli.commands.archive.cmd_archive",
        "trace": "fstdd.cli.commands.trace.cmd_trace",
        "install": "fstdd.cli.commands.install.cmd_install",
        "rollback": "fstdd.cli.commands.rollback.cmd_rollback",
        "diff": "fstdd.cli.commands.diff.cmd_diff",
        "abort": "fstdd.cli.commands.abort.cmd_abort",
        "extract-proposal": "fstdd.cli.commands.extract_proposal.cmd_extract_proposal",
        "dependency-graph": "fstdd.cli.commands.dependency_graph.cmd_dependency_graph",
        "ci": "fstdd.cli.commands.ci.cmd_ci",
        "experience": "fstdd.cli.commands.experience.cmd_experience",
        "state": "fstdd.cli.commands.state.cmd_state",
        "gate": "fstdd.cli.commands.gate.cmd_gate",
        # V2.7 new commands
        "proposal": "fstdd.cli.commands.proposal._dispatch",
        "canon": "fstdd.cli.commands.canon._dispatch",
        "index": "fstdd.cli.commands.index._dispatch",
        "agent": "fstdd.cli.commands.agent._dispatch",
        "hooks": "fstdd.cli.commands.hooks._dispatch",
        "structure": "fstdd.cli.commands.structure._dispatch",
        "skill": "fstdd.cli.commands.skill._dispatch",
        # V2.8 new commands
        "fix": "fstdd.cli.commands.fix._dispatch",
        # V2.9 new commands
        "upgrade": "fstdd.cli.commands.upgrade.cmd_upgrade",
        "batch": "fstdd.cli.commands.batch.cmd_batch",
        # V2.9.3+ new commands
        "guard": "fstdd.cli.commands.guard.cmd_guard",
        "phase": "fstdd.cli.commands.phase.cmd_phase",
