...
> STDD (Spec+Test Driven Development) V3.0.5
...
### 4.2 四阶段流程 / Four-Phase Flow

```
Phase 1: /stdd-understand  →  需求理解 / Understand → proposal.md → 用户确认 / User confirm (Gate 1)
Phase 2: /stdd-spec        →  规格设计 / Spec design → design.md + specs + test-plan.md → 用户确认 / User confirm (Gate 2)
Phase 3: /stdd-continue    →  BUILD（V3.0.5：SLICE/BUILD/VERIFY 合一）→ 切片规划 + TDD实现 + 质量验证(11类失败模式+E2E+覆盖率) → 用户确认 / User confirm (Gate 3)
Phase 4: /stdd-continue    →  交付 / Deliver：归档 / Archive + 合并 specs / Merge specs + git tag
```
...
| `/stdd-continue` | 继续执行（Phase 3-4）/ Continue execution |
...
gates:
  phase1_understand:
    required: true          # Phase 1 必须用户确认 / User confirmation required (Gate 1)
  phase2_spec:
    required: true          # Phase 2 必须用户确认 / User confirmation required (Gate 2)
  phase3_build:
    required: true          # Phase 3 必须用户确认 / User confirmation required (Gate 3, V3.0.5: 原 phase5_verify)
