...
`.fstdd.yaml` 被 20+ 个读写端共享，字段名不一致（如 `current_phase` vs `phase`）会导致静默 bug。当前无自动检测机制。

## 方案

### Canonical 字段定义

定义 `.fstdd.yaml` 的 canonical schema 文件 `stdd/schema/stdd_yaml_schema.yaml`：
...
```bash
stdd schema verify                # 校验当前项目的 .fstdd.yaml
...
2. **类型检查**：读取实际 .fstdd.yaml 的值类型，比对 schema
