...
⚠️ **Gate 2 前自检** — 以下 5 项全部存在才能展示 Gate 2：

```
[ ] design.md       — 技术设计（Context/Decisions/Architecture/Risks）
[ ] specs/ 目录      — 每个 capability 一个 spec.md（GIVEN/WHEN/THEN/SHALL）
[ ] test-plan.md    — TC-ID 映射 + 覆盖矩阵 + 回归风险
[ ] Canonical YAML  — proposal.yaml + spec.yaml
[ ] Scenario ≥ Requirement（每个 Requirement ≥1 Scenario）
```
...
用户确认后：
1. 写入 `design.md`
2. **V3.0.5 (YAML-first)**：确保 Canonical YAML 已就绪（AI 直接写 YAML，MD 由 Gate 2 自动生成）：
   - `canonical/proposals/<change>.yaml`（Step 1/3 已就绪）
   - `canonical/specs/code/<capability>.yaml`（Step 4b 已生成，每个 capability 一个）
   - `canonical/specs/agent/<capability>.yaml`（Step 4c 已生成）
   - 无需手动写 `specs/<capability>/spec.md`，也无需重复执行 `canon generate --all`
3. 写入 `test-plan.md`
...
Phase 2 文档已锁定。在进入 Phase 3 之前，**必须**选择 Phase 3 的执行模式。此步骤不可跳过。
...
🚀 全自动长程模式 [推荐]
   ...
   · Phase 3（BUILD：切片规划 + TDD 实现 + 质量验证）连续自动执行
...
5. **Step 8a.5: 配置 Claude Code 实际权限**（长程模式关键步骤）：
   ...
   - 使用 Edit 工具在 `permissions.allow` 数组中添加以下规则：
     ...
     - 搜索规则：`Glob(**)`, `Grep(**)`
     - Skill 规则：`Skill(stdd-build)`, `Skill(stdd-deliver)`
...
6. 用户确认全部授权后：
   - 更新 `.stdd.yaml`，记录长程模式状态：
     ...
     ```
   - 提示用户："长程模式已启用，Phase 3 将全自动连续执行，仅在 Gate 3 等待确认。"
   - 进入 Phase 3: BUILD
...
1. 更新 `.stdd.yaml`：
   ...
   long_range:
     ...
     mode: "normal"
   ```
2. 提示用户："普通交互模式，Phase 3 将在需要时暂停交互。"
3. 进入 Phase 3: BUILD
...
- [ ] 用户已选择 Phase 3 执行模式（长程/普通）

## 下一阶段

Phase 2 确认完成 + 模式选择完成 → 进入 Phase 3: BUILD（切片规划 + TDD 实现 + 质量验证）
