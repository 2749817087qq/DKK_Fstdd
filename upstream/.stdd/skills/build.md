...
**V2.9 轻量模式**：如果 `.stdd.yaml` 中 `mode: lightweight`，跳过切片规划，使用 1 个隐式切片，直接进入 Part B。
...
2. 读取 `.stdd/standards/<language>.md`
3. **加载 `.stdd/rules/`**：读取 `.stdd/rules/common/*.md` 和 `.stdd/rules/<language>/*.md`
...
| C4 | 失败模式检查 | 全部检查完成 |
| C5 | 经验库自动记录/更新 | 失败模式已记录到 .stdd/experiences/ |
...
在运行自动化质量检查之前，**必须先执行多路并行技术评审**。读取 `.stdd/config.d/quality.yaml` 中的 `review` 配置，启动 3 个并行评审代理（代码、测试、文档），收集审查发现并逐条处置（修复 / 记录为已知问题）。
...
将失败模式检查中发现的新模式自动记录到 `.stdd/experiences/`，更新既有经验的命中次数。
