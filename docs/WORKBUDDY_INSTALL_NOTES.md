...
2. **路径固化**：skill 正文里 `.stdd/skills/_shared/*.md`、`python bin/fstdd` 均为项目相对路径，
...
- **联网升级**：`fstdd-upgrade` 会从 `raw.githubusercontent.com` 拉取文件覆盖本地 `.stdd/` 静态资源，
  属用户主动触发行为，未做改动。
...
- `init`：成功，生成 `.stdd/` 骨架、FSTDD.md、AGENTS.md、FSTDD_CONSTITUTION.md，Guard 已激活
