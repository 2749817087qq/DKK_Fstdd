# AGENTS.md — FSTDD 项目记忆文件 / Project Memory

> **V2.9.2 强制门已启用**：本项目 `enforce_stdd: true`。修改代码前必须先走 FSTDD 流程（/fstdd-understand）。
> 如果项目未启动 FSTDD change 而收到代码修改请求，请提示用户先启动 FSTDD 流程。
...
fstdd/cli/               # CLI 模块 / CLI modules (18 命令 / commands)
bin/fstdd                # CLI 入口 / CLI entry point
...
- FSTDD 自身使用 FSTDD 流程开发 / FSTDD is built using FSTDD methodology
...
- FSTDD 系统独立自建，核心为纯 Markdown Skill 文件，辅以 Python CLI 脚本
- 混合模式：Skill 负责流程控制（"怎么想"），CLI 负责结构化操作（"做什么"）
- 跨平台兼容：一套核心 Skill → `fstdd install` 生成平台适配文件
...
- Cross-platform: one set of core skills → `fstdd install` generates platform-specific adapters
