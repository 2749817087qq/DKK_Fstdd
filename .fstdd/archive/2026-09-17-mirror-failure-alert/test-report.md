...
## 十、勘误（交付后复核发现，2026-09-17 深夜）

> 本节**不删改上文**，而是如实记录两处事实错误及其更正 —— 保留错误痕迹本身有价值。

### 勘误 1：`test_a6` **并未**「提交后自动恢复」

§六 原文写：「提交后该断言自动恢复 —— 它失败的内容恰好反证了改动范围完全在预期内」。

**实测不成立。** 全部改动提交后，该断言**仍然失败**，失败列表变成了 18 个
`.fstdd/archive/` 下的文件。原报告把失败归因为「未提交改动」，
是**把现象当成了根因** —— 提交只能消除「未提交」这一类原因，不能消除其它原因。

### 勘误 2：那 18 个文件**不是 EOL 混合态**，而是 index **stat 缓存过期**

§7.1b 与 §7.2 把它们称为「既存混合态文件（`i/lf w/crlf`）」，
并据此提议「跑 `verify_eol.py --fix` 归一」。**该诊断是错的**，按它去修会
**白白重写 18 个本来正确的文件**。

真实情况：**三个副本逐字节完全相同**。

```
$ 工作区 bytes=2528  CRLF=0   sha256[:12]=6fb4179c595e
$ HEAD    bytes=2528  CRLF=0   sha256[:12]=6fb4179c595e
$ index   bytes=2528  CRLF=0   sha256[:12]=6fb4179c595e
  git hash-object = fb5764dd78ea
  HEAD:  blob id  = fb5764dd78ea
  index blob id   = fb5764dd78ea
```

判据来自 `git ls-files --debug`：

```
  ctime: 1789606892:350866200
  mtime: 1789614048:896757600      <-- 早于工作区实际 mtime
  size: 2602                       <-- CRLF 时代的字节数
```

工作区文件已是 **2528** 字节（LF），而 index 记录的 stat 是 **2602**（CRLF）。
2602 − 2528 = **74**，正好等于 74 个 CRLF。**stat 对不上 → `git status` 报 M**，
尽管内容一字未改。

### 修法与效果

修法只是 `git add` 这 18 个文件 —— **只刷新 stat，零内容变更**：

| 核验项 | 结果 |
|---|---|
| `git diff --cached --name-only` | **空**（无内容变更） |
| index blob id | 未变（仍 `fb5764dd…`） |
| 是否需要新提交 | **否**（无内容变更可提交） |
| `test_migrate_to_d_drive.py` | **19 passed**（原 1 failed / 18 passed） |
| `tools/verify_eol.py` | **7 / 7 通过**（原 5 / 7） |

**§7.1b 的「校验脚本现状」据此更正**：`verify_eol.py` 的两个红叉中，
TC-EOL-003 是**误诊**（真因是 stat 缓存，不是 EOL）；TC-EOL-005 也一并转绿 ——
该用例只在**已提交**的树上才有意义（工作区有未提交改动时它必然报「非预期变更」）。

**§7.2 第 5 项（archive 18 个混合态文件）随之关闭** —— 无需另立 change 做 EOL 归一，
因为**根本没有 EOL 问题**。TC-EOL-005 判据仍建议按 §7.1 第 5 项改为
「普通 add vs renormalize add」的比较（更强判据），但那是**改进**而非修 bug。
...
cd D:/tools/FSTDD/stdd-repo/upstream
C:/Python311/python.exe -m pytest tests -q
→ 640 passed, 1 skipped in 383.62s (0:06:23)
```

> **教训**：**「`git status` 说 `M`」不等于「内容变了」**。
> 判定必须用「工作区字节 vs HEAD blob 哈希」比对，而不是看状态字母。
> 这与我此前反复强调的「看起来生效」是**同一类错误的镜像**：
> 那次是「看起来生效、其实没生效」，这次是「看起来变了、其实没变」。
> 两者都只能靠**独立于表面信号的第二判据**识别。
