...
> 本表**前三行**即原设计中的三个待议点，**已裁定，不再阻塞 Gate 2**；
> **ISO-4** 为 BUILD 阶段（Slice 3）新增裁定。
...
| **ISO-4**（BUILD 阶段新增，Slice 3） | **模板源恒取主仓** `project_root`，不取 `target_root` | ① worktree 是 HEAD 的**签出**，主仓里**未提交**的模板不会随行；若模板源改取 `target_root`，这类模板会被 `if tmpl.exists()` **静默跳过**，产出缺 `design.md` / `test-plan.md` 的半成品骨架。② 模板是**项目级**资源，worktree 只是 change 的落点，源在语义上就该是主仓。落地即「`templates_dir` 一行不动」= 零漂移；由 TC-ISO-001b 锚定（断言复制到的内容 = 主仓未提交版本，可区分两种实现） |
